...
test('Square render() method returns correct SVG string', () => {
  const shape = new Square(); // Create a Square instance
  shape.setColor('red'); // Set the color of the Square to 'red'
  shape.setText('ABC'); // Set the text value
  ...
  const renderedSvg = shape.render().replace(/\s/g, ''); // Remove all whitespace characters from the rendered SVG string
  const expectedSvgWithoutWhitespace = expectedSvg.replace(/\s/g, ''); // Remove all whitespace characters from the expected SVG string
